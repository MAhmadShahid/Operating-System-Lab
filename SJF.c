...
void sortByBurstTime(struct Process * process, int numberOfProcess)
{
    struct Process key;
    int j;
    ...
    for ( int i = 1; i < numberOfProcess; i++)
    {
        ...
        while( j >= 0 && (process[j].cpuBurstTime>key.cpuBurstTime))
        {
            process[j+1] = process[j];
            j--;
        }
